...
client.on('message', message => { 
    ...
    if (command == "role") {
    if (!message.channel.guild) return;
    if(!message.guild.member(message.author).hasPermission("MANAGE_ROLES")) return message.reply("** MANAGE_ROLES انت لا تملك برمشن  **").then(msg => msg.delete(5000));;
    if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply(" MANAGE_ROLES البوت لا يملك برمشن  ").then(msg => msg.delete(5000));;
    ...
    message.reply('*** تم اضافة الرتبة الى الشخص :white_check_mark:  ***').then(msg => {msg.delete(10000)});
    }
    });
