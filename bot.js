...
client.on('message', message => {
    ...
    message.channel.send("<@" + message.author.id + ">  *** جاري اعطاء الرتبة الى الأعضاء  *** ")
    message.guild.members.forEach(m => {
    m.addRole(message.guild.roles.find('name', 'Member'))
    })
    }
    
       if (message.content ===   prefix + "remove role") {
    message.channel.send("<@" + message.author.id + ">  ***  جاري ازالة الرتبة من الأعضاء  *** ")
    message.guild.members.forEach(m => {
    m.removeRole(message.guild.roles.find('name', 'Member'))
    })
    }
